...
The Mesh-Coupled XOR TRNG proposed by Lu et. al. \cite{lu_ultra-high_2025} is an autonomous boolean network designed 
based on first principles established by Gong et. al. \cite{gong_unpredictability_2020} that fundamentally uses 
several ring oscillators, coupled XOR gates, and a mesh network topology to generate a chaotic-like entropy source. 
The autonomous boolean network mechanism, similar to work done by Rosin \cite{rosin_dynamics_2015} and Zhang and 
collaborators \cite{zhang_boolean_2009}, allows for the use of nonlinear dynamics and chaos to expose the hardware 
fabric's manufacturing variance as an entropy source. 
...
present, though it is assumed due to its similarity to work by Rosin \cite{rosin_dynamics_2015} and 
Zhang \cite{zhang_boolean_2009}.

The entropy source samples from several points within the boolean network, specifically at the left- and
right-hand sides of the coupled XOR units \cite{lu_ultra-high_2025}. Six sets of four measurement points 
are then defined, and each of the four measurement points are XORed together to generate six bits of randomness.
...
both using the same modeling technique as Rosin \cite{rosin_dynamics_2015} and Gong et. al. 
\cite{gong_unpredictability_2020}, and then with a new technique that used slew rates as opposed to first 
order low-pass filtering. The recreation with the former technique is provided in figure 
\ref{fig:mesh_coupled_xor_plde} and the latter technique in \ref{fig:mesh_coupled_xor_plsr}.
